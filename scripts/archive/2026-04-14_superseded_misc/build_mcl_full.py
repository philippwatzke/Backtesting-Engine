#!/usr/bin/env python
"""Build full-session MCL train, validation, or test splits with extended intraday coverage."""
import argparse
...
RAW_PATH = Path("data/raw/MCL_1m_2022_heute_raw.parquet")
TRAIN_SPLIT_PATH = Path("data/processed/MCL_1m_train.parquet")
VALIDATION_SPLIT_PATH = Path("data/processed/MCL_1m_validation.parquet")
TEST_SPLIT_PATH = Path("data/processed/MCL_1m_test.parquet")
TRAIN_OUTPUT_PATH = Path("data/processed/MCL_1m_full_train.parquet")
VALIDATION_OUTPUT_PATH = Path("data/processed/MCL_1m_full_val.parquet")
TEST_OUTPUT_PATH = Path("data/processed/MCL_1m_full_test.parquet")
SESSION_START = "08:00"
SESSION_END = "15:59"
...
def _build_full_split(
    ...
    if split_df.empty:
        raise ValueError(f"No MCL raw rows available inside the requested {split_name} session window")
...
def main() -> None:
    ...
    for path in (RAW_PATH, TRAIN_SPLIT_PATH, VALIDATION_SPLIT_PATH, TEST_SPLIT_PATH):
        if not path.exists():
            raise FileNotFoundError(f"Required MCL file not found: {path}")
