#!/usr/bin/env python
"""Explore the M6A overnight train split on 5-minute bars."""
...
def explore(parquet_path: Path, atr_period: int, session_start: str, session_end: str) -> dict:
    ...
    outlier_threshold = hotspot_mean + (2.0 * hotspot_std)
    ...
    outliers = hotspot_df[hotspot_df["avg_range_ticks"] >= outlier_threshold].copy()
    ...
    return {
        ...
        "volatility_hotspots": {
            "top_10_times": hotspot_df.head(10).to_dict(orient="records"),
            "outlier_threshold_ticks": outlier_threshold,
            "outlier_times": outliers.to_dict(orient="records"),
        },
...
def main() -> None:
    ...
    parser.add_argument("--data", type=Path, default=Path("data/processed/M6A_1m_overnight_train.parquet"))
    parser.add_argument("--atr-period", type=int, default=14)
    parser.add_argument("--session-start", type=str, default="18:00")
    parser.add_argument("--session-end", type=str, default="02:00")
